...
#final_vars <- c(player_pos, d_team_dummy, d_pos_dummy)
final_vars <- c(player_pos, d_team_dummy)
...
for (i in seq(1:length(unq_player_pos))) {
  ...
  p_discontin_index <- which(d_team_dummy == paste0(this_player_pos, '_discontin'))
  ...
  add.constraint(
    ...
    type = ">=",
    rhs = 5,
    indices = c(player_indices, length(player_pos) + p_discontin_index)
  ) 
}
...
final_obj <- c(
  player_stat_value,
  #team discon constraint
  rep(0, length(d_team_dummy))
  #pos discon constraint
  #rep(0, length(d_pos_dummy))
)
...
Set the bounds for the team dummy discon variables and the position discon
```{r}

dummy_start <- length(player_only) 
team_dummy_seq <- c((dummy_start + 1):(dummy_start + num_players))

# pos_dummy_start <- dummy_start + num_players
# pos_dummy_seq <- c((pos_dummy_start + 1):(pos_dummy_start + num_roster))

set.semicont(best, col = team_dummy_seq)
# set.semicont(best, col = pos_dummy_seq)

set.bounds(best, upper = rep(5, length(team_dummy_seq)), col = team_dummy_seq)
# set.bounds(best, upper = rep(5, length(pos_dummy_seq)), col = pos_dummy_seq)
